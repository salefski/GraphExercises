...
void topographialSortClass::initializeSort (graphDatabaseClass* g) {
    ...
    while(!sortStack.empty()) sortStack.pop();
...
//
void topographialSortClass::processVertexEarly (int v) {
    if(noisy) {
        printf("Process Vertex Early %s\n", graphToProcess->getVertexName(v).c_str());
    }
}

void topographialSortClass::processVertexLate (int vertexID) {
    if(noisy) {
        printf("Process Vertex Late %s\n", graphToProcess->getVertexName(vertexID).c_str());
    }
    sortStack.push(vertexID);
 }

void topographialSortClass::processEdge (int x, int y, int w) {
    if(noisy) {
...
topographialSortClass::edgeClassification topographialSortClass::classifyEdge (int x, int y) {
    //
...
//
edgenode* topographialSortClass::getNextEdgeToProcess(std::deque<edgenode*>& edgeDeque) {
...
//
bool topographialSortClass::checkForCycle (int x, int y) {
...
//
void topographialSortClass::printEdgeClassification (edgeClassification edgeIsA) {
...
// the verticies
//
void topographialSortClass::topSortReport() {
    int x = 0; // vertex
    edgenode* p = nullptr; // temporary
    std::vector<int>::iterator itr;
    
    printf("Sorted list is:");
    for(itr=sortedVector.begin(); itr<sortedVector.end(); ++itr) {
        printf(" %s", graphToProcess->getVertexName(*itr).c_str());
    }
    printf("\n\n");
...
// depth first search
//
void topographialSortClass::dfs (int vertexID) {
...
void topographialSortClass::topSort(int start) {
    std::vector<vertexRecord>::iterator itr;
    
    for(int i = 0; i < graphToProcess->numVertices(); ++i) {
        if(!vertexInfo[i].discovered) dfs(i);
    }
    
    while(!sortStack.empty()) {
        sortedVector.push_back(sortStack.top());
        sortStack.pop();
    }
}
